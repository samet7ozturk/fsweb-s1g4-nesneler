...
console.log(MenuElemaniOlustur("Karışık Pizza", 100, "Pizzalar"));
console.log(MenuElemaniOlustur("Et Dürüm", 70, "Dürümler"));
console.log(MenuElemaniOlustur("Köri Soslu Makarna", 60, "Makarnalar"));
...
/*function DegerlendirmeEkle(isim, puan, geribildirim) {
  /*Kodlar buraya */

function DegerlendirmeEkle(liste, a, b, c) {
  let newObject = {};
  newObject.isim = a;
  newObject.puan = b;
  newObject.geribildirim = c;
  liste.push(newObject);
  return liste;
}
/*  Görev 6: 
...
/*function AnahtardanDegerlendirmeAl(değerlendirmesi,ind) {

  console.log(değerlendirmeler[ind].isim + "isimli kişi" + değerlendirmeler[ind].puan + "puan verdi ve şunları yazdı: " + değerlendirmeler[ind].geribildirim);
}*/
...
function SonDegerlendirmeyiAl(liste) {
  const text = `${liste[liste.length - 1].isim} isimli kişi ${
    degerlendirmeler[liste.length - 1].puan
  } puan verdi ve şunları yazdı: ${
    degerlendirmeler[liste.length - 1].geribildirim
  }`;
  return text;
}
